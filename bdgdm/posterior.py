...
def get_posterior_summary_single(summary):
    """
    Convert a flat BDGDM posterior-summary dictionary into tidy tables.

    Parameters
    ----------
    summary : dict
        Output of the BDGDM posterior summarization step.
    ...
    parameters : pandas.DataFrame
        Main posterior parameter summaries.

    transitions : pandas.DataFrame
        Transition-level posterior summaries.
    """
    subtype = summary.get(
        "subtype_label_s1",
        "single_group",
    ...
    parameter_definitions = [
        ("b0", "b0_s1"),
        ("b_scaling", "b_scaling_s1"),
        ("b_deviation", "b_deviation_s1"),
    ...
    for parameter, prefix in parameter_definitions:
        ...
        transition_rows.append(
            {
                ...
                "lp_median": summary.get(
                    f"lp_{transition}_s1_median"
                ),
                "lp_q025": summary.get(
                    f"lp_{transition}_s1_q025"
                ),
                "lp_q975": summary.get(
                    f"lp_{transition}_s1_q975"
                ),
                "fractional_change_median": summary.get(
                    f"fracCN_{transition}_s1_median"
                ),
                "fractional_change_q025": summary.get(
                    f"fracCN_{transition}_s1_q025"
                ),
                "fractional_change_q975": summary.get(
                    f"fracCN_{transition}_s1_q975"
                ),
                "p_positive": summary.get(
                    f"p_fracCN_{transition}_pos_s1"
                ),
                "p_negative": summary.get(
                    f"p_fracCN_{transition}_neg_s1"
                ),
                "p_rope": summary.get(
                    f"p_rope_fracCN_{transition}_s1"
                ),
                "ppd": summary.get(
                    f"ppd_fracCN_{transition}_s1"
                ),
                "scaling_component_median": summary.get(
                    f"lp_scaling_{transition}_s1_median"
                ),
                "deviation_component_median": summary.get(
                    f"lp_dev_{transition}_s1_median"
                ),
                "cancellation_index_median": summary.get(
                    f"cancel_index_{transition}_s1_median"
                ),
            }
        )
